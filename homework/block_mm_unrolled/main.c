    ...
    for (int i = 0; i < rows_a; i += BLOCK_SIZE)
    {
        for (int j = 0; j < cols_b; j += BLOCK_SIZE)
        {
            for (int k = 0; k < cols_a; k += BLOCK_SIZE)
            {
                ...
                {
                    for (int bj = 0; bj < block_size_j; bj += 4)
                    {
                        float sum0 = 0.0f, sum1 = 0.0f, sum2 = 0.0f, sum3 = 0.0f;
                        for (int bk = 0; bk < block_size_k; bk++)
                        {
                            float a = input0->data[(i + bi) * cols_a + (k + bk)];
                            float b0 = input1->data[(k + bk) * cols_b + (j + bj)];
                            float b1 = (j + bj + 1 < cols_b) ? input1->data[(k + bk) * cols_b + (j + bj + 1)] : 0.0f;
                            float b2 = (j + bj + 2 < cols_b) ? input1->data[(k + bk) * cols_b + (j + bj + 2)] : 0.0f;
                            float b3 = (j + bj + 3 < cols_b) ? input1->data[(k + bk) * cols_b + (j + bj + 3)] : 0.0f;
                        ...
                        result->data[(i + bi) * cols_b + (j + bj)] += sum0;
                        if (j + 1 < cols_b) result->data[(i + bi) * cols_b + (j + bj + 1)] += sum1;
                        if (j + 2 < cols_b) result->data[(i + bi) * cols_b + (j + bj + 2)] += sum2;
                        if (j + 3 < cols_b) result->data[(i + bi) * cols_b + (j + bj + 3)] += sum3;
                    }
                }
            }
        }
...
// #define EPSILON 1e-4  // Relative tolerance
// #define ABSOLUTE_EPSILON 1e-5  // Absolute tolerance for near-zero values
